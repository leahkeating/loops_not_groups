...
# Same as loops_no_groups_theory.py except T_loop = T and T_group = T2
# (group reinforcement on, loop reinforcement off)
...
for num_tri in num_tri_vec:
    ...
    np.savetxt("data/size_groups_no_loops_num_tri_{}_upper_branch.txt".format(int(num_tri)), supercrit[:, :2], fmt="%.6f")
    output_data = np.column_stack((Tlist, Prob))
    np.savetxt("data/prob_groups_no_loops_num_tri_{}.txt".format(int(num_tri)),
                    output_data, fmt="%.6f")
    ...
    np.savetxt("data/groups_no_loops_rho_boundary_num_tri_{}.txt".format(int(num_tri)), np.column_stack((T_vals, rho_boundary)), fmt="%.6f")
    ...
    np.savetxt("data/size_groups_no_loops_num_tri_{}_lower_branch.txt".format(int(num_tri)), np.column_stack((lower_T, np.zeros_like(lower_T))), fmt="%.6f")
