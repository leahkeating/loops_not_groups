...
for mu,nu in zip(mu_vec, nu_vec):
    ...
    np.savetxt("data/size_simple_contagion_nu_{}_upper_branch.txt".format(int(nu)), supercrit[:, :2], fmt="%.6f")
    output_data = np.column_stack((Tlist, Prob))
    np.savetxt("data/prob_simple_contagion_nu_{}.txt".format(int(nu)),
                    output_data, fmt="%.6f")
    ...
    np.savetxt("data/size_simple_contagion_nu_{}_lower_branch.txt".format(int(nu)), subcrit_plot[:, :2], fmt="%.6f")
    ...
    np.savetxt("data/simple_contagion_rho_boundary_nu_{}.txt".format(int(nu)), np.column_stack((T_vals, rho_boundary)), fmt="%.6f")
