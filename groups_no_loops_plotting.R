...
theory_size_lower_3 <- read_delim("data/size_groups_no_loops_nu_3_lower_branch.txt",
                            col_names = c("T", "p"),
                            delim = " ") |> mutate(nu = factor(3))

theory_size_lower_2 <- read_delim("data/size_groups_no_loops_nu_2_lower_branch.txt",
                            col_names = c("T", "p"),
                            delim = " ") |> mutate(nu = factor(2))

theory_size_lower_1 <- read_delim("data/size_groups_no_loops_nu_1_lower_branch.txt",
                            col_names = c("T", "p"),
                            delim = " ") |> mutate(nu = factor(1))

theory_size_upper_3 <- read_delim("data/size_groups_no_loops_nu_3_upper_branch.txt",
                            col_names = c("T", "p"),
                            delim = " ") |> mutate(nu = factor(3))

theory_size_upper_2 <- read_delim("data/size_groups_no_loops_nu_2_upper_branch.txt",
                            col_names = c("T", "p"),
                            delim = " ") |> mutate(nu = factor(2))

theory_size_upper_1 <- read_delim("data/size_groups_no_loops_nu_1_upper_branch.txt",
                            col_names = c("T", "p"),
                            delim = " ") |> mutate(nu = factor(1))

theory_rho_boundary_3 <- read_delim("data/groups_no_loops_rho_boundary_nu_3.txt",
                            col_names = c("T", "p"),
                            delim = " ") |> mutate(nu = factor(3))

theory_rho_boundary_2 <- read_delim("data/groups_no_loops_rho_boundary_nu_2.txt",
                            col_names = c("T", "p"),
                            delim = " ") |> mutate(nu = factor(2))

theory_rho_boundary_1 <- read_delim("data/groups_no_loops_rho_boundary_nu_1.txt",
                            col_names = c("T", "p"),
                            delim = " ") |> mutate(nu = factor(1))

theory_prob_3 <- read_delim("data/prob_groups_no_loops_nu_3.txt",
                            col_names = c("T", "p"),
                            delim = " ") |> mutate(nu = factor(3))

theory_prob_2 <- read_delim("data/prob_groups_no_loops_nu_2.txt",
                            col_names = c("T", "p"),
                            delim = " ") |> mutate(nu = factor(2))

theory_prob_1 <- read_delim("data/prob_groups_no_loops_nu_1.txt",
                            col_names = c("T", "p"),
                            delim = " ") |> mutate(nu = factor(1))

sims_3 <- read_delim("data/groupsnoloops_nu_3_sims.txt",
                     col_names = c("T", "cascade_size"),
                     delim = " ") |> mutate(nu = factor(3)) |>
  mutate(rho = cascade_size/100000)

sims_3_near_boundary <- read_delim("data/logs_2000000/groupsnoloops_nu_3_sims.txt",
                     col_names = c("T", "cascade_size"),
                     delim = " ") |> mutate(nu = factor(3)) |>
...
sims_3 <- sims_3 |> filter((T < 0.06) | (T > 0.15)) |>
  add_row(sims_3_near_boundary |> filter(T >= 0.06, T %in% near_boundary_T))

sims_2 <- read_delim("data/groupsnoloops_nu_2_sims.txt",
                     col_names = c("T", "cascade_size"),
                     delim = " ") |> mutate(nu = factor(2)) |>
  mutate(rho = cascade_size/100000)

sims_1 <- read_delim("data/groupsnoloops_nu_1_sims.txt",
                     col_names = c("T", "cascade_size"),
                     delim = " ") |> mutate(nu = factor(1)) |>
  mutate(rho = cascade_size/100000)
