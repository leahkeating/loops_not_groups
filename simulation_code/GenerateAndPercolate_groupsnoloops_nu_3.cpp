...
// Parameters
const int N = 100000;
const double lambda_edges = 0.5;
const double lambda_triangles = 3.0;
...
int dynamic_percolation(int seed_node, double T1, double T2) {
    ...
    std::unordered_map<std::array<int, 3>, int, Array3Hash> triangle_transmission;
    ...
    while (!q.empty()) {
        ...
        for (const auto& tri : triangles_per_node[u]) {
            ...
            for (auto [x, y] : edges) {
                ...
                double T = T1;
                if(triangle_exposure[tri].count(other) == 1 && triangle_transmission[tri]==1) T = T2; //was the other node already exposed once?
                if (!occupied_nodes[other] && rand_uniform() < T) {
                    triangle_transmission[tri] = 1; //track local transmissions
