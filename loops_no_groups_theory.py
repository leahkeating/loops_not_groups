...
for mu,nu in zip(mu_vec, nu_vec):
    ...
    for T in reversed(Tlist):
        ...
        v_in2 = rho_in1**2*(1-T)*(1-T2)+2*rho_in1*(1-rho_in1)*(1-T)*((1-T)+T*(1-T2))+(1-rho_in1)**2
        v_out2 = 2*T*(1-T)*(1-T2)*Gt(u_out1,v_out1,mu,nu)+(2*T*(1-T)*T2+T**2)*(Gt(u_out1,v_out1,mu,nu)**2)+(1-T)**2
    ...
    np.savetxt("size_loops_not_groups_mu_{}.txt".format(mu),
                    output_data, fmt="%.6f")
    output_data = np.column_stack((Tlist, Prob))
    np.savetxt("prob_loops_not_groups_mu_{}.txt".format(mu),
                    output_data, fmt="%.6f")