# Loops and groups theory code for probability and size
# Same as loops_no_groups_theory.py except T_group = T2 (group reinforcement on)
...
for num_tri in num_tri_vec:
    ...
    np.savetxt("data/size_loops_and_groups_num_tri_{}_upper_branch.txt".format(int(num_tri)), supercrit[:, :2], fmt="%.6f")
    output_data = np.column_stack((Tlist, Prob))
    np.savetxt("data/prob_loops_and_groups_num_tri_{}.txt".format(int(num_tri)),
                    output_data, fmt="%.6f")
    ...
    np.savetxt("data/loops_and_groups_rho_boundary_num_tri_{}.txt".format(int(num_tri)), np.column_stack((T_vals, rho_boundary)), fmt="%.6f")
    ...
    np.savetxt("data/size_loops_and_groups_num_tri_{}_lower_branch.txt".format(int(num_tri)), np.column_stack((lower_T, np.zeros_like(lower_T))), fmt="%.6f")
