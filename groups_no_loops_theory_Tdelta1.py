# Groups, no loops theory code for probability and size
# Same as groups_no_loops_theory.py except T_group (T_Delta) = 1
# (group reinforcement fully on, loop reinforcement off)
...
def one_step(state, T, mu, nu):
    """Apply your map once and return (next_state, delta_rho)."""
    rho, u, v = state
    T_loop = T
    T_group = 1.0
    u2 = (1.0 - T) + T*Gs(u, v, mu, nu)
    v2 = (rho**2)*(1.0-T)*((1.0-T)*(1.0-T_loop)+T*(1.0-T_group))+2*rho*(1.0-rho)*(1.0-T)*((1.0-T)+T*(1.0-T_group))+(1.0-rho)**2
    rho2 = 1.0 - Gt(u2, v2, mu, nu)
    return np.array([rho2, u2, v2], dtype=float), (rho2 - rho)

def stabilize_ic(state, T, mu, nu, n=2):
    """Optionally pre-iterate a couple times to 'stabilize' ICs like you did."""
    s = np.array(state, dtype=float)
    for _ in range(n):
        s, _ = one_step(s, T, mu, nu)
    return s

def split_state_for_T(T, upper_branch_lhs, lower_branch_rhs, mu, nu,
    ...
    up = stabilize_ic(up0, T, mu, nu, n=warmup)
    lo = stabilize_ic(lo0, T, mu, nu, n=warmup)

    # Evaluate directions
    _, dup = one_step(up, T, mu, nu)
    _, dlo = one_step(lo, T, mu, nu)
...
for mu,nu in zip(mu_vec, nu_vec):
    ...
    np.savetxt("data/size_groups_no_loops_Tdelta1_nu_{}_upper_branch.txt".format(int(nu)), supercrit[:, :2], fmt="%.6f")
    output_data = np.column_stack((Tlist, Prob))
    np.savetxt("data/prob_groups_no_loops_Tdelta1_nu_{}.txt".format(int(nu)),
                    output_data, fmt="%.6f")
    ...
    np.savetxt("data/size_groups_no_loops_Tdelta1_nu_{}_lower_branch.txt".format(int(nu)), subcrit_plot[:, :2], fmt="%.6f")
    ...
    for T in T_vals:
        state_star = split_state_for_T(T, upper_branch_lhs, lower_branch_rhs,
                                    mu, nu, tol=tol, maxit=maxit, warmup=2)
        rho_boundary.append(state_star[0])  # rho component (may be NaN if not bracketed)

    np.savetxt("data/groups_no_loops_Tdelta1_rho_boundary_nu_{}.txt".format(int(nu)), np.column_stack((T_vals, rho_boundary)), fmt="%.6f")
